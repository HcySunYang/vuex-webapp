...
;(function(win, doc, undefined){
	...
	$.extend(proto, {
		...
		bindSlide : function(){
			...
			$(self.settings.contentSelete).off('touchcancel');
			...
			$(self.settings.contentSelete).on('touchcancel', function(event){
				// 解决手掌胡乱
				$('.product-title').html('cancel')
				self.bindSlide();
		...
		contentTouchEndFn : function(event, obj){
			...
			$('.product-title').html($('.product-title').html() + self.disX)
			if(self.lockX){
				if(-10 < self.disX){
					self.bindSlide();
